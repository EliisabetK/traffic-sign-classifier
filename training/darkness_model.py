...
from testing import metamorphic_test
...
def process_img(img_path, noise_p=0.7, blur_p=0.05, dark_p=0.9, dark_f=0.9):
    ...
    if np.random.rand() < dark_p:
        dark_factor = random.uniform(0.2, dark_f)
        img = np.clip(img * dark_factor, 0, 1)
    return img
...
for i in range(25):
    ...
    temp_path = f"{output_dir}/temp_dark_model_{i}.h5"
    ...
    if darkness_consistency >= 90 and brightness_consistency < 85 and saturation_consistency < 88 and noise_consistency > 87 and blur_consistency < 85:
        os.rename(temp_path, f"{output_dir}/dark.h5")
        print("Saved dark.h5")
        break
else:
