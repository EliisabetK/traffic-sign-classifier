...
def augment_with_random_darkening_and_noise(generator, X, y, batch_size=32, noise_factor=0.15):
    while True:
        for X_batch, y_batch in generator.flow(X, y, batch_size=batch_size):
            darkened_X_batch = np.array([darken_image(img, random.uniform(0.6, 1.8)) for img in X_batch])
...
train_generator = augment_with_random_darkening_and_noise(train_datagen, X_train, y_train)

val_datagen = ImageDataGenerator()
val_generator = augment_with_random_darkening_and_noise(val_datagen, X_val, y_val)
